...
async function handleEvent(event) {
  ...
  // ------------------------------------------------------
  // ⭐ OVERRIDE LOGIC (แก้ใหม่ให้ฉลาดขึ้น)
  // ------------------------------------------------------
  const hsMatch = keyword.match(/\b\d{6}\b/);
  if (hsMatch) {
    const hsCode = hsMatch[0];

    // หา user message ก่อนหน้าที่ไม่ใช่เลข 6 หลัก
  ...
  if (results.length > 0) {
    ...
    listForAI = sliced.map((item, index) => {
      return `• HS: ${item.hsCode} | TH: ${item.th} | EN: ${item.en} | อากร: ${item.no} | FE: ${item.fe}`;
    }).join('\n');
...
${sliced.map((item, index) => {
  ...
  // ------------------------------------------------------
  // ⭐ AI PROMPT (ผนวก prompt ใหม่ของคุณ)
  // ------------------------------------------------------
  let prompt = "";
...
2) วิเคราะห์พิกัดใหม่ตามหลักเกณฑ์ศุลกากรไทย โดยใช้ความรู้ของคุณเอง แม้ข้อมูลในฐานข้อมูลจะไม่ครบก็ตาม แบบสรุปสั้นๆ
